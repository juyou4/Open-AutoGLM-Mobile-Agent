...
> 这是一个直接运行在 Android 手机上的 AI 操作助手。用户输入自然语言任务后，App 在本机截屏、调用 AutoGLM 视觉模型、解析动作，再通过 Android 无障碍服务（AccessibilityService）执行点击、输入、滑动和返回。敏感操作会先停下来确认。
>
> 项目参考 [Open-AutoGLM](https://github.com/zai-org/Open-AutoGLM)，保留它的 Agent 闭环、动作协议、模型和坐标系统，只把执行层从「PC + ADB」换成「手机 + AccessibilityService」。

## 当前 MVP 状态

本目录已经是可打开的 Android 原生 MVP 工程，具体实现范围见 [MVP_NOTES.md](MVP_NOTES.md)。
...
也可以直接用 Android Studio 打开本目录，连接 Android 11+ 设备，安装 Debug APK 后在系统设置里开启“AutoGLM 手机 Agent”无障碍服务。
...
对应任务：`打开设置，进入 Wi-Fi 页面`。实测流程是 App 截图 → 请求 `autoglm-phone` → 执行 `Launch(系统设置)` → 二次截图识别 WLAN 页面 → `finish(message=...)`。
...
| 作业要求 | 这里怎么满足 | 对应章节 |
|---|---|---|
| **① 基于 Open-AutoGLM** | 复用 system prompt、`do()/finish()` 动作协议、0–999 相对坐标和单截图上下文；默认调用智谱 `autoglm-phone` | §2 §4 §5 §6 |
| **② 利用 vibe coding** | 按 Observation / Model / Action / Safety / Trace 分层，用 AI 编程逐模块生成并迭代，保留 prompt 和迭代记录 | §10 |
| **③ 移动端 App** | Kotlin + Jetpack Compose 原生 Android 应用，可打包成 APK 侧载安装 | §3 §8 |
| **④ 能在手机上运行** | 截图、模型调用和动作执行都在手机本机完成，不依赖 PC，也不依赖 ADB | §3 §5 §6 |

> 和「手机当遥控器、PC 跑 Open-AutoGLM」不同，这里的 Agent 闭环跑在手机上。

---

## 1. 为什么不是直接打包 Open-AutoGLM

读过上游源码后可以确认，Open-AutoGLM 是运行在电脑或服务器上的 Python 框架，执行层依赖 `adb` / `hdc` / WebDriverAgent 三套子进程命令（`phone_agent/adb/`、`phone_agent/hdc/`、`phone_agent/xctest/`）。它不能直接打包成手机 App。
...
system prompt、动作协议、坐标系和 Agent Loop 都保留，所以仍然是“基于 Open-AutoGLM”。
...
1. **单截图上下文**：每步把上一条 user 消息里的图片删掉，历史里始终只保留当前这一张截图（上游 `agent.py:205` `remove_images_from_message`）。历史只保留文本化的 `<think>../<answer>..`。
2. **assistant 历史格式**：模型回复按 `"<think>{thinking}</think><answer>{action}</answer>"` 拼回 assistant 消息再入历史（上游 `agent.py:220-224`）。
...
配置页也可以切换到任意 OpenAI-compatible 多模态模型（如 GLM-4V）。此时用自定义 JSON schema 输出（`{"thought":...,"action":{...}}`）。通用 VLM 的坐标 grounding 通常弱于 AutoGLM，所以 demo 默认还是走 4.1。
...
以及坐标系：**左上 (0,0) 到右下 (999,999)**。这套不要改，不然就接不住 `autoglm-phone` 的输出。
...
> 流式（`stream:true`）可选：先跑通非流式，再加流式显示 think。
...
UI 树（`getRootInActiveWindow()`）不喂给模型（AutoGLM 是纯视觉），只在执行端使用：把模型给的坐标对齐到最近的可点击节点、做兜底定位、判断是否敏感控件。
...
- **权限说明**：首启展示无障碍用途说明并取得用户同意；项目定位为研究/学习用途，APK 侧载，不上架 Google Play。

---

## 10. vibe coding 工作流（作业要求②，要留痕）

这里的 vibe coding 不是让 AI 一次性生成整包代码，而是拆层后逐步生成、逐步验收：
...
4. **保留过程**：保存关键 prompt、迭代对话和关键 diff，方便回看实现是怎么收敛的。

---

## 11. MVP 范围与开发排期

> 原则：先把一条任务端到端跑通，再补其他能力。
...
验收：输入「打开设置，进入 WiFi 页面」，App 自主截图、请求模型、执行动作，多步完成并展示每步 think/action。
...
- [Open-AutoGLM](https://github.com/zai-org/Open-AutoGLM) — 参考了 Agent Loop、动作协议、system prompt、模型调用、敏感确认/人工接管。
